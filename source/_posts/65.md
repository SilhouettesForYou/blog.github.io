...
如果使用`MyPair(5, "hello");`会导致编译失败，从构造函数的角度都无法让数组类型衰退为指针。从 C++ 11 开始使用`std::decay`主动让数组类型衰退为指针，而在 C++ 11 之前，都是使用传值的方法来达到让数组类型衰退为指针
...
``` c++
// 标准库中的形式
template<typename _T1, _T2> pair(_T1, _T2) -> pair<_T1, _T2>;
// 类似地
MyPair(int, const char*) -> MyPair<long long, std::string>;
template<typename _T1, _T2> MyPair(_T1, _T2) -> MyPair<_T1, _T2>;
```

如果有这么一句指引就会按照指引的方式推导，如果能够匹配到优先走自定义推导（相对于模板特化）

* 聚合类型模板的推导指引 {% label primary @C++ 20 %}

在 C++ 20 标准发布之前聚合类型的类模板是无法进行模板实参推导的
...
C++ 20 之前编译会报错，为了让代码编译通过，可以显示地指定模板实参：
...
SFINAE (Substitution Failure Is Not An Error, 替换失败不是错误)，模板形参的替换有两个时机：

* 在模板推导的最开始阶段，当明确地指定替换模板形参的实参进行替换
* 在模板推导的最后，模板形参会根据实参进行推导或使用默认的模板实参

主要是区分编译错误和替换失败的情况，而定义编译错误比定义替换失败更容易，所以只提出了编译错误的情况

* 处理表达式外部某些实体时发生的错误，比如实例化某模板或生成某隐式定义的成员函数等
* 由于实现限制导致的错误
* 由于访问违规导致的错误
* 由于同一个函数的不同声明的词法顺序不同，导致替换顺序不用或者根本无法替换产生的错误
...
int main() {
  X x1, x2;
  X x3 = foo(x1, x2); // X 没有重载 + 运算符，所以替换失败了
}
```
...
template <int I> void foo(char(*)[I % 2 == 0] = 1) {
...
SIFINAE 的一个典型的应用就是标准库中的`std::enable_if`模板元函数
...
### 概念 {% label primary @C++ 20 %}

在 C++ 20 中可以改写为

``` c++
template <class C>
concept IntegetType = std::is_integral_v<C>;
...
* `IntegerType`是概念名，`std::is_integral_v<C>`是约束表达式；如果表达式计算结果位`true`，那么该实参满足约束条件；这里的计算都是编译期执行的，概念的最终结果是一个`bool`类型的纯右值
* 约束表达式还支持一般的逻辑操作，包括合取与析取

### 原子约束

* 原子约束是表达式和表达式中模板形参到模板实参的组合（简称形参映射）
* 当约束表达式中存在原子约束时，如果约束表达式结果相同，则约束表达式应该是相同的，否则会编译失败
...
逻辑否定表达式是一个原子约束。所以以上代码会产生二义性，原子约束表达式`!sad<T>`并不来自相同的约束表达式。为了让代码能成功编译，需要修改为：

``` c++
template <class T> concept sad = false;
template <class T> concept not_sad = !sad<T>;
template <class T> int f1(T) requires (not_sad) { return 1; };
template <class T> int f1(T) requires (not_sad) && true { return 2; };
f1(0); // 编译成功
```

### `requires`表达式

对`requires`表达式进行模板实参提花，如果替换之后`requires`表达式中出现无效类型或者表达式违反约束条件，则`requires`表达式求值为`false`，反之则`requires`表达式求值为`true`
...
`s1<int, double, std::string>`，其约束实际上为`(C1<int> && C1<double> && C1<std::string>)`
...
* 外部模板
  * 链接器对于模板有特殊待遇，编译器在编译每份源代码的时候会按照单个源代码的需要生成模板实例，而链接器对于这些实例会进行一次去重操作，它将完全相同的实例删除，最后只留下一份实例
* 连续右尖括号的解析优化
  * 两个连续的右尖括号`>>`一定会被编译器解析为右移
  * C++ 11 把解析原则改了
* `friend`声明模板形参
* 变量模板
* `explicit(bool)`